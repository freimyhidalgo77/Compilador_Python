"""
CAPA SINTÁCTICA — FASE 2 (sintactico) + FASE 3 (semantico embebido)
─────────────────────────────────────────────────────────────
"""
...
class AnalizadorSintactico:
    ...
    def analizar(self):
        ...
        while self._actual().tipo != TipoToken.FIN_ARCHIVO:
            self._parsear_declaracion()
        # El ámbito global se deja abierto a propósito para poder imprimir
        # la tabla de símbolos completa al terminar el análisis.
    ...
    def _parsear_declaracion(self):
        ...
        if t.tipo == TipoToken.RESERVADA:
            manejador = {
                'var': self._parsear_var,
                'if': self._parsear_if,
                'while': self._parsear_while,
                'for': self._parsear_for,
                'function': self._parsear_function,
                'return': self._parsear_return,
                'break': lambda: self._parsear_salto('break'),
                'continue': lambda: self._parsear_salto('continue'),
                'begin': self._parsear_bloque,
            }.get(t.valor)
            if manehador := manejador:
                manehador()
                return
        if t.tipo == TipoToken.IDENTIFICADOR:
            self._parsear_asignacion()
            return
        self._err_sint(t, f'declaracion inesperada "{t.valor}"')
        self._avanzar()

    def _parsear_var(self):
        linea_decl = self._actual().linea
        ...
        valor_final = None
        if self._actual().tipo == TipoToken.ASIGNACION:
            self._avanzar()
            ...
            if tipo_dato != TipoDato.DESCONOCIDO and tipo_expr != TipoDato.DESCONOCIDO and tipo_dato != tipo_expr:
                self._err_sem(id_tok, f'no se puede asignar un valor de tipo {tipo_expr} a la '
                                       f'variable "{id_tok.valor}" declarada como {tipo_dato}')
                valor_final = None
        self._consumir(TipoToken.PUNTO_COMA, '";"')

        if id_tok is not None:
            ok = self.tabla_simbolos.declarar(Simbolo(id_tok.valor, tipo_dato, valor_final, False, linea_decl))
            if not ok:
    ...
    def _parsear_asignacion(self):
        ...
        if simb is not None and not simb.es_constante:
            if simb.tipo != TipoDato.DESCONOCIDO and tipo_expr != TipoDato.DESCONOCIDO and simb.tipo != tipo_expr:
    ...
    # ---- mini-evaluador constante, igual que evaluarBinaria/evaluarRelacional en Kotlin ----
    @staticmethod
